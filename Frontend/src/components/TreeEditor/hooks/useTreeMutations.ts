...
export const useTreeMutations = (
    ...
    (nodeOrId?: TreeNode | string) => {
      const node =
        typeof nodeOrId === 'string'
          ? tree.findById(nodeOrId, treeOptions.idPropertyName)
          : nodeOrId;
      if (node) {
        ...
        setDeletions([
          ...
          {
            parentId:
              parent?.getData()[treeOptions.idPropertyName] ?? UNDEFINED_ID,
  ...
  const recoverDeletion = React.useCallback((): TreeNode | undefined => {
    ...
    if (deletions.length > 0) {
      ...
      const parentNode =
        tree.findById(deletionInfo.parentId, treeOptions.idPropertyName) ??
        tree;
  ...
  const updateNode = React.useCallback(
    (nodeOrId: TreeNode | string, dataPartial: AnyObject) => {
      const node =
        typeof nodeOrId === 'string'
          ? tree.findById(nodeOrId, treeOptions.idPropertyName)
          : nodeOrId;
  ...
  const addSiblingNodeData = React.useCallback(
    (nodeOrId: TreeNode | string, data: AnyObject) => {
