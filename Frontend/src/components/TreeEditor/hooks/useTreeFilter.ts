...
export const useTreeFilter = (
  ...
  const [filteredNodeMap, setFilteredNodeMap] = React.useState<
    Record<string, TreeNode>
  >({});
  const [filteredAncestorNodeMap, setFilteredAncestorNodeMap] = React.useState<
    Record<string, TreeNode>
  >({});
    ...
    (nodeOrId?: TreeNode | string) => {
      const id =
        typeof nodeOrId === 'string'
          ? nodeOrId
          : (nodeOrId?.getData()[treeOptions.idPropertyName] ?? UNDEFINED_ID);
  ...
  React.useEffect(() => {
    if (hasFilter) {
      ...
      const filteredNodes = tree.findAll(
        (node) => !!filterRef.current && filterRef.current(node),
      );
      ...
      allNodes.forEach((node) => {
        for (let i = 0; i < filteredNodes.length; i++) {
          ...
          if (node.isAncestorOf(currFilteredNode)) {
            newFilteredAncestorIdMap[
              node.getData()[treeOptions.idPropertyName]
            ] = currFilteredNode;
            break;
          }
        }
