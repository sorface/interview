...
export const useTreeFocus = (
  ...
  const isFocusedId = React.useCallback(
    (id?: string) => typeof id !== 'undefined' && focusedId === id,
    [focusedId],
  );
  const isFocusedNode = React.useCallback(
    (treeNode?: TreeNode) =>
      focusedNode &&
      treeNode &&
      typeof focusedNode.getData()[treeOptions.idPropertyName] === 'string' &&
      typeof treeNode.getData()[treeOptions.idPropertyName] === 'string' &&
      focusedNode.getData()[treeOptions.idPropertyName] ===
        treeNode.getData()[treeOptions.idPropertyName],
  ...
  const focusUp = React.useCallback(() => {
    ...
    if (focusedNode) {
      ...
      if (leftSibling) {
        ...
        while (
          expansions.isExpandedNode(youngestChild) &&
          youngestChild.hasChildren()
        ) {
          youngestChild =
            youngestChild.getChildren()[youngestChild.getChildren().length - 1];
        }
        upFocusId =
          youngestChild.getData()[treeOptions.idPropertyName] ?? UNDEFINED_ID;
      }
      // Otherwise, if there's a parent, focus that
      else if (focusedNode.getParent()) {
        upFocusId =
          focusedNode.getParent()?.getData()[treeOptions.idPropertyName] ??
          UNDEFINED_ID;
      }
  ...
  const focusDown = React.useCallback(() => {
    ...
    if (focusedNode) {
      ...
      if (expansions.isExpandedNode(focusedNode) && focusedNode.hasChildren()) {
        downFocusId =
          focusedNode.getChildren()[0].getData()[treeOptions.idPropertyName] ??
          UNDEFINED_ID;
      }
      // If there's an immediate right sibling, focus that
      else if (focusedNode.getRightSibling()) {
        downFocusId =
          focusedNode.getRightSibling()?.getData()[
            treeOptions.idPropertyName
          ] ?? UNDEFINED_ID;
      }
      // Otherwise, find the immediate right sibling to the closest ancestor and focus that
        ...
        while (parent) {
          ...
          if (parentRightSibling) {
            downFocusId =
              parentRightSibling.getData()[treeOptions.idPropertyName] ??
              UNDEFINED_ID;
            break;
          } else {
