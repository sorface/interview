...
export const QuestionCreate: FunctionComponent<QuestionCreateProps> = ({
  ...
  const { rootCategory: rootCategoryParam, subCategory: subCategoryParam } =
    useParams();
  const { apiMethodState: questionState, fetchData: fetchCreateQuestion } =
    useApiMethod<Question['id'], CreateQuestionBody>(
      questionsApiDeclaration.create,
    );
  const {
  ...
  const {
    apiMethodState: updatingQuestionState,
    fetchData: fetchUpdateQuestion,
  } = useApiMethod<Question, UpdateQuestionBody>(
    questionsApiDeclaration.update,
  );
  const {
  ...
  const [codeEditor, setCodeEditor] = useState<Question['codeEditor'] | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    toast.success(
      localizationCaptions[LocalizationKey.QuestionCreatedSuccessfully],
    );
    onClose();
  ...
  useEffect(() => {
    ...
    toast.success(
      localizationCaptions[LocalizationKey.QuestionUpdatedSuccessfully],
    );
    onClose();
  ...
  return (
    <ModalWithProgressWarning
      warningCaption={
        localizationCaptions[LocalizationKey.CurrentQuestionNotBeSaved]
      }
      contentLabel={
        editQuestionId
          ? localizationCaptions[LocalizationKey.EditQuestion]
    ...
    >
      <div className="question-create text-left">
        ...
        <QuestionCreateField.Wrapper>
          <QuestionCreateField.Label>
            <label htmlFor="qestionText">
              {localizationCaptions[LocalizationKey.QuestionText]}:
            </label>
          </QuestionCreateField.Label>
          <QuestionCreateField.Content className="flex">
            <input
              ...
              {answers.map((answer) => (
                <div key={answer.id}>
                  <div className="question-answer-controls flex items-center">
                    <input
                      ...
                      placeholder={
                        localizationCaptions[
                          LocalizationKey.QuestionAnswerOptionName
                        ]
                      }
                      value={answer.title}
                    ...
                    >
                      <Typography size="m">
                        {
                          localizationCaptions[
                            LocalizationKey.QuestionDeleteAnswerOption
                          ]
                        }
                      </Typography>
                    </div>
                  ...
                  <CodeEditor
                    ...
                    language={
                      answer.codeEditor && codeEditor?.lang
                        ? codeEditor.lang
                        : CodeEditorLang.Plaintext
                    }
                    languages={
                      codeEditor && codeEditor.lang !== CodeEditorLang.Plaintext
                        ? [CodeEditorLang.Plaintext, codeEditor.lang]
                        : [CodeEditorLang.Plaintext]
                    }
                    alwaysConsumeMouseWheel={false}
                    scrollBeyondLastLine={false}
        ...
        {admin && (
          <div>
            <Gap sizeRem={1.5} />
            <div>
              <label htmlFor="qestionType">
                {localizationCaptions[LocalizationKey.QuestionType]}:
              </label>
            </div>
            <select id="qestionType" value={type} onChange={handleTypeChange}>
              <option value={QuestionType.Private}>
                {localizationCaptions[LocalizationKey.QuestionTypePrivate]}
              </option>
              <option value={QuestionType.Public}>
                {localizationCaptions[LocalizationKey.QuestionTypePublic]}
              </option>
            </select>
          </div>
        )}
        <ModalFooter>
          ...
          <Button
            variant="active"
            onClick={editQuestionId ? handleSubmitEdit : handleSubmitCreate}
          >
            {editQuestionId
