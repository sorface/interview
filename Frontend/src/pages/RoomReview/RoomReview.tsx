...
export const sortMyRoomReviews = (
  r1: MyRoomQuestionEvaluation,
  r2: MyRoomQuestionEvaluation,
) => r1.order - r2.order;
...
export const RoomReview: FunctionComponent = () => {
  ...
  const [openedAnswerDetailsId, setOpenedAnswerDetailsId] = useState<
    string | null
  >(null);
  ...
  } = useApiMethod<MyRoomQuestionEvaluation[], Room['id']>(
    roomQuestionEvaluationApiDeclaration.getMy,
  );
  const {
    process: {
      loading: myQuestionEvaluationsLoading,
      error: myQuestionEvaluationsError,
    },
    data: myQuestionEvaluations,
  } = getMyQuestionEvaluationsState;

  const { apiMethodState: getMyRoomReviewState, fetchData: fetchMyRoomReview } =
    useApiMethod<RoomReviewType, Room['id']>(roomReviewApiDeclaration.getMy);
  const {
    process: { loading: myRoomReviewLoading, error: myRoomReviewError },
    data: myRoomReview,
  } = getMyRoomReviewState;

  const {
    apiMethodState: completeRoomReviewState,
    fetchData: fetchCompleteRoomReview,
  } = useApiMethod<RoomReviewType, CompleteRoomReviewsBody>(
    roomReviewApiDeclaration.complete,
  );
  const {
    process: {
      loading: completeRoomReviewLoading,
      error: completeRoomReviewError,
    },
    data: completedRoomReview,
  } = completeRoomReviewState;

  const {
    apiMethodState: upsertRoomReviewState,
    fetchData: fetchUpsertRoomReview,
  } = useApiMethod<RoomReviewType, UpsertRoomReviewsBody>(
    roomReviewApiDeclaration.upsert,
  );
  const {
    process: { loading: upsertRoomReviewLoading, error: upsertRoomReviewError },
    data: upsertedRoomReview,
  } = upsertRoomReviewState;
  ...
  const reviewCompleted = myRoomReview
    ? myRoomReview.state === 'Closed'
    : false;
  const shouldRedirectToAnalytics =
    !canWriteReview ||
    reviewCompleted ||
    completedRoomReview ||
    roomCloseCode === HttpResponseCode.Ok;
  ...
  const allUsers = room
    ? getAllUsers(room)
    : new Map<User['id'], Pick<User, 'nickname' | 'avatar'>>();
  ...
  if (loading || !room || myQuestionEvaluationsLoading || myRoomReviewLoading) {
    return (
      <>
        <PageHeader
          title={localizationCaptions[LocalizationKey.RoomReviewPageName]}
        />
        <Loader />
      </>
    );
      ...
      <div className="overflow-auto">
        ...
        <InfoBlock className="text-left flex flex-col">
          <Typography size="s" bold>
            {localizationCaptions[LocalizationKey.CandidateOpinion]}
          </Typography>
          <Gap sizeRem={1} />
          <Textarea
            className="h-3.625"
        ...
        <InfoBlock className="text-left">
          <Typography size="xl" bold>
            {localizationCaptions[LocalizationKey.CandidateMarks]}
          </Typography>
          <Gap sizeRem={2} />
          {myQuestionEvaluations &&
            myQuestionEvaluations
              .sort(sortMyRoomReviews)
              .map((questionEvaluations, index, allMyQuestionEvaluations) => (
                <Fragment key={questionEvaluations.id}>
                  <RoomReviewQuestionEvaluation
                    roomId={room.id}
                    questionEvaluations={questionEvaluations}
                    onDetailsOpen={() =>
                      handleDetailsOpen(questionEvaluations.id)
                    }
                  />
                  {index !== allMyQuestionEvaluations.length - 1 && (
                    <Gap sizeRem={0.25} />
                  )}
                </Fragment>
              ))}
        ...
        >
          <QuestionAnswerDetails
            ...
            questionTitle={
              myQuestionEvaluations?.find((e) => e.id === openedAnswerDetailsId)
                ?.value || ''
            }
            allUsers={allUsers}
          />
        ...
        >
          ...
          />
          <ModalFooter>
            <Button onClick={handleCloseSaveModal}>
        ...
        <div className="flex justify-end">
          ...
          <Button variant="active" onClick={handleOpenSaveModal}>
            {completeRoomReviewLoading ? (
              <Loader />
            ) : (
