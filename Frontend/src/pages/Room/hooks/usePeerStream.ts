...
export type UsePeerStreamState = Map<
  string,
  { audio?: MediaStream; video?: MediaStream; loaded?: boolean }
>;
...
export const usePeerStream = () => {
  const [peerToStream, dispatch] = useReducer(
    usePeerStreamReducer,
    initialState,
  );

  const addPeerStream = useCallback(
    (peerId: string, peer: Peer.Instance, trackLoading: boolean) => {
      dispatch({
        name: 'setPeerLoaded',
        payload: { peerId, loaded: !trackLoading },
      });
